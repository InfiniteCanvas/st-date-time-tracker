<script>
    ...
    // Properly initialize default arrays BEFORE passing to $state if they are missing
    ...
    // Now they are guaranteed to exist before becoming reactive
    let global = $state({ ...extState.global });
    let chat = $state({ ...extState.chat });

    onMount(() => {
        const syncChat = () => { chat = { ...extState.chat }; };
        const syncGlobal = () => { global = { ...extState.global }; };
    ...
    function addCustomAdjustment() {
        if (newAdjAmount > 0) {
            updateGlobal('customAdjustments', [...global.customAdjustments, { amount: newAdjAmount, unit: newAdjUnit }]);
        }
    }
...
<div use:portal class="flex flex-col gap-3 p-2 text-sm">
    <label class="flex items-center gap-2 font-bold">
        <input type="checkbox" checked={global.isEnabled} onchange={(e) => updateGlobal('isEnabled', e.target.checked)} />
        Enable Extension
    </label>

    <div class="flex flex-col gap-1">
        <b>Current Date & Time (Saved per Chat)</b>
        <input type="datetime-local" class="text_pole w-full" value={dtLocalStr()} onchange={handleDateChange} />
    </div>

    <div class="flex flex-col gap-1">
        <b>Auto-Advance (Minutes per turn)</b>
        <input type="number" class="text_pole w-24" value={chat.autoAdvanceMinutes} onchange={(e) => updateChat('autoAdvanceMinutes', Number(e.target.value))} />
    </div>

    <div class="flex flex-col gap-1">
        <b>Prompt Format</b>
        <input type="text" class="text_pole w-full" value={chat.promptFormat} onchange={(e) => updateChat('promptFormat', e.target.value)} />
    </div>
    ...
    <div class="flex flex-col gap-1">
        ...
        <div class="flex gap-2 mb-2 w-full">
            <input type="text" placeholder="Label (e.g., Midnight)" class="text_pole" style="flex: 1 1 auto; min-width: 0;" bind:value={newBtnLabel} />
            <input type="number" placeholder="Hr" class="text_pole" style="width: 70px; flex: 0 0 auto;" bind:value={newBtnHour} min="0" max="23"/>
            <button class="menu_button m-0" style="flex: 0 0 auto;" onclick={addCustomButton}>Add</button>
        </div>
        ...
        <div class="flex flex-col gap-1">
            {#each global.customButtons as btn, i}
                <div class="flex justify-between items-center bg-black/20 p-2 rounded border border-white/10">
                    ...
                    <button class="text-red-400 hover:text-red-300 px-2 py-1 bg-black/30 rounded cursor-pointer" onclick={() => removeCustomButton(i)}>
    ...
    <div class="flex flex-col gap-1">
        ...
        <div class="flex gap-2 mb-2 w-full">
            <input type="number" class="text_pole" style="flex: 1 1 auto; min-width: 0;" bind:value={newAdjAmount} min="1"/>
            <select class="text_pole" style="width: 75px; flex: 0 0 auto;" bind:value={newAdjUnit}>
                <option value="m">mins</option>
                <option value="h">hrs</option>
                <option value="d">days</option>
            </select>
            <button class="menu_button m-0" style="flex: 0 0 auto;" onclick={addCustomAdjustment}>Add</button>
        </div>
        ...
        <div class="flex flex-col gap-1">
            {#each global.customAdjustments as adj, i}
                <div class="flex justify-between items-center bg-black/20 p-2 rounded border border-white/10">
                    ...
                    <button class="text-red-400 hover:text-red-300 px-2 py-1 bg-black/30 rounded cursor-pointer" onclick={() => removeCustomAdjustment(i)}>
                        <i class="fa-solid fa-trash"></i>
                    </button>
                </div>
            {/each}
...
{#if global.showWidget && global.isEnabled}
    ...
    >
        <div class="flex flex-col gap-1 border-b border-white/20 pb-2 cursor-grab active:cursor-grabbing" onmousedown={onDragStart}>
        ...
        {#if global.customButtons.length > 0}
            <div class="grid grid-cols-2 gap-1 mt-1">
                {#each global.customButtons as btn}
                    <button class="menu_button m-0 text-xs py-1" onclick={() => setTimeOfDay(btn.hour)}>{btn.label}</button>
                {/each}
            </div>
            ...
            <div class="flex flex-col gap-1">
                {#each global.customAdjustments as adj}
                    <div class="flex gap-1">
                        <button class="menu_button m-0 flex-1 text-xs py-1" onclick={() => adjustTime(adj.unit, -adj.amount)}>
                            -{adj.amount}{adj.unit}
                        </button>
                        <button class="menu_button m-0 flex-1 text-xs py-1" onclick={() => adjustTime(adj.unit, adj.amount)}>
                            +{adj.amount}{adj.unit}
